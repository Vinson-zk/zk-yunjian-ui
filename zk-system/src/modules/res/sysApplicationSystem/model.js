...
import { editSysApplicationSystem, delSysApplicationSystem, getSysApplicationSystem, findSysApplicationSystems } from './service';

import { zkTools } from 'zkFramework';
const { zkToolsUtils, zkToolsMsg } = zkTools;

const model = {
    namespace: 'mSysApplicationSystem',
    state: {
    ...
    effects: { // action
        // 编辑 
        *editSysApplicationSystem({ payload, callback }, { call }) {
            let res = yield call(editSysApplicationSystem, payload);
            let f = errors=>{
        ...
        // 删除
        *delSysApplicationSystem({ payload, callback }, { call }) {
            let res = yield call(delSysApplicationSystem, payload);
            if(res.code == "zk.0"){
                zkToolsMsg.alertMsg(null, null, {type:"success", msg:res.msg});
            }
            if (callback instanceof Function) {
        ...
        // 查询 详情
        *getSysApplicationSystem({ payload }, { call, put }) {
            let res = yield call(getSysApplicationSystem, payload);
            if (res.code == 'zk.0') {
         ...
         */
        *findSysApplicationSystems({ filter, page, sorter, callback }, { call, put, select }) {
            ...
            let res = yield call(findSysApplicationSystems, params);
            let restState = {}
            if (res.code == 'zk.0') {
                restState = {
                    "filter": params,
