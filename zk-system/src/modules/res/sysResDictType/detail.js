...
class CInitSysResDictTypeDetail extends Component {
	...
	render() {
		...
		let { location, mApp, mSysResDictType, intl, loading } = this.props;
		let { optEntity  } = mSysResDictType;
		let lang = mApp.lang?mApp.lang:zkToolsMsg.getLocale();

		let spinning = loading.effects['mSysResDictType/getSysResDictType'];

		return (optEntity != null && mSysResDictType.pathname == location.pathname) && (
			<ZKSpin spinning={spinning === true} >
				<ZKDetailGrid >
		            <ZKDetailGrid.Row>
						<ZKDetailGrid.ColLabel>{zkToolsMsg.msgFormatByIntl(intl, 'zk.sys.res.SysResDictType.typeCode')}:</ZKDetailGrid.ColLabel>
						<ZKDetailGrid.ColValue>
							{optEntity.typeCode}
						</ZKDetailGrid.ColValue>
					</ZKDetailGrid.Row>       
		            <ZKDetailGrid.Row>
						<ZKDetailGrid.ColLabel>{zkToolsMsg.msgFormatByIntl(intl, 'zk.sys.res.SysResDictType.typeName')}:</ZKDetailGrid.ColLabel>
						<ZKDetailGrid.ColValue>
							<ZKInputJson disabled styleType="compact" value={optEntity.typeName?optEntity.typeName:{}} primaryAttr={lang} attrs={locales} />
						</ZKDetailGrid.ColValue>
						<ZKDetailGrid.ColLabel>{zkToolsMsg.msgFormatByIntl(intl, 'zk.sys.res.SysResDictType.typeDesc')}:</ZKDetailGrid.ColLabel>
						<ZKDetailGrid.ColValue>
    ...
    componentDidMount() {
        let { location, dispatch, match, mSysResDictType } = this.props;
		let { params } = match;
		// 第一次进来或地址栏改变了 
...
export default injectIntl(connect(({ mApp, mSysResDictType, loading }) => ({ mApp, mSysResDictType, loading }))(CInitSysResDictTypeDetail));