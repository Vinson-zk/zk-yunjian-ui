 ...
 * @Last Modified time: 2024-06-24 17:42:49
 */

import { editSysResDictType, delSysResDictType, getSysResDictType, findSysResDictTypes } from './service';

import { zkTools } from 'zkFramework';
const { zkToolsUtils, zkToolsMsg } = zkTools;

const model = {
    namespace: 'mSysResDictType',
    state: {
    ...
    effects: { // action
        // 编辑 
        *editSysResDictType({ payload, callback }, { call }) {
            let res = yield call(editSysResDictType, payload);
            let f = errors=>{
        ...
        // 删除
        *delSysResDictType({ payload, callback }, { call }) {
            let res = yield call(delSysResDictType, payload);
            if(res.ok){
                zkToolsMsg.alertMsg(null, null, {type:"success", msg:res.msg});
            }
            if (zkJsUtils.assertObjType(callback, Function)) {
        ...
        // 查询 详情
        *getSysResDictType({ payload }, { call, put }) {
            let res = yield call(getSysResDictType, payload);
            if (res.ok) {
         ...
         */
        *findSysResDictTypes({ filter, pagination, sorter, callback }, { call, put, select }) {
