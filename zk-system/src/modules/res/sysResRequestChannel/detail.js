...
class CInitSysResRequestChannelDetail extends Component {
	...
	render() {
		...
		return (
			<ZKSpin spinning={spinning === true} >
				<ZKDetailGrid >
		            <ZKDetailGrid.Row>
						<ZKDetailGrid.ColLabel>{zkToolsMsg.msgFormatByIntl(intl, 'zk.sys.res.SysResRequestChannel.code')}:</ZKDetailGrid.ColLabel>
    ...
    componentDidMount() {
        let { location, dispatch, match, mSysResRequestChannel } = this.props;
		let { params } = match;
		// 第一次进来或地址栏改变了 
...
export default injectIntl(connect(({ mApp, mSysResRequestChannel, loading }) => ({ mApp, mSysResRequestChannel, loading }))(CInitSysResRequestChannelDetail));