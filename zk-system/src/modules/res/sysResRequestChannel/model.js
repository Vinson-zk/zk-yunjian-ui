 ...
 * @Last Modified time: 2024-06-24 17:43:18
 */

import { editSysResRequestChannel, delSysResRequestChannel, getSysResRequestChannel, findSysResRequestChannels } from './service';

import { zkTools } from 'zkFramework';
const { zkToolsUtils, zkToolsMsg } = zkTools;

const model = {
    namespace: 'mSysResRequestChannel',
    state: {
    ...
    effects: { // action
        // 编辑 
        *editSysResRequestChannel({ payload, callback }, { call }) {
            let res = yield call(editSysResRequestChannel, payload);
            let f = errors=>{
        ...
        // 删除
        *delSysResRequestChannel({ payload, callback }, { call }) {
            let res = yield call(delSysResRequestChannel, payload);
            if(res.ok){
                zkToolsMsg.alertMsg(null, null, {type:"success", msg:res.msg});
            }
            if (zkJsUtils.assertObjType(callback, Function)) {
        ...
        // 查询 详情
        *getSysResRequestChannel({ payload }, { call, put }) {
            let res = yield call(getSysResRequestChannel, payload);
            if (res.ok) {
         ...
         */
        *findSysResRequestChannels({ filter, pagination, sorter, callback }, { call, put, select }) {
