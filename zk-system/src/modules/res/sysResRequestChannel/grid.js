...
const f_getTableColumns = (onEedit, onDetail, onDelete, intl) => {
	...
	return [
		{
			title: zkToolsMsg.msgFormatByIntl(intl, 'zk.sys.res.SysResRequestChannel.name'),
		...
		{
			title: zkToolsMsg.msgFormatByIntl(intl, 'zk.sys.res.SysResRequestChannel.code'),
			textAlign: 'center', dataIndex: 'code', key: 'code', width: 100, 
		},
		{
			title: zkToolsMsg.msgFormatByIntl(intl, 'zk.sys.res.SysResRequestChannel.channelDesc'),
			textAlign: 'center', dataIndex: 'channelDesc', key: 'channelDesc', width: 100, 
			render: (text, record, index) => {
				return zkToolsMsg.getInternationInfo(record.channelDesc?record.channelDesc:{}, intl.locale);
			}
		},
		{
...
class CInitSysResRequestChannelGrid extends React.Component {
	...
	f_changeSelKeys = (selRowKeys, selRows) => {
		this.props.dispatch({ type: "mSysResRequestChannel/setState", payload: { gridSelKeys: selRowKeys } });
	}
	...
	render() {
		...
		let { intl, mApp, mSysResRequestChannel, loading } = this.props;

		let tableColumns = f_getTableColumns(this.f_edit, this.f_detail, this.f_delete, intl);

		let gridLoading = loading.effects['mSysResRequestChannel/findSysResRequestChannels'];
		...
		return (
			<ZKScrollTable loading = { gridLoading }
				...
				rowSelection = {{
					...
					selectedRowKeys: mSysResRequestChannel.gridSelKeys||[], 
					columnWidth: '32px'
				}}
				rowKey = "pkId"
			...
			>
				<ZKOptRow>
					<ZKOptRow.OptGroup>
						...
						<ZKOptRow.OptGroup.OptItem onClick={(e) => {
							// 删除
							this.f_delete(mSysResRequestChannel.gridSelKeys, true);
...
export default CInitSysResRequestChannelGrid;