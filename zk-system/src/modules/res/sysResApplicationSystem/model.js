 ...
 * @Last Modified time: 2024-06-24 17:42:23
 */
...
const model = {
    namespace: 'mSysResApplicationSystem',
    state: {
    ...
    effects: { // action
        // 编辑 
        *editSysResApplicationSystem({ payload, callback }, { call }) {
            let res = yield call(editSysResApplicationSystem, payload);
            let f = errors=>{
        ...
        // 删除
        *delSysResApplicationSystem({ payload, callback }, { call }) {
            let res = yield call(delSysResApplicationSystem, payload);
            if(res.ok){
                zkToolsMsg.alertMsg(null, null, {type:"success", msg:res.msg});
            }
            if (zkJsUtils.assertObjType(callback, Function)) {
        ...
        // 查询 详情
        *getSysResApplicationSystem({ payload }, { call, put }) {
            let res = yield call(getSysResApplicationSystem, payload);
            if (res.ok) {
