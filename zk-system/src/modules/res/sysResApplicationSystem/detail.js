...
class CInitSysResApplicationSystemDetail extends Component {
	...
	render() {
		...
		return (
			<ZKSpin spinning={spinning === true} >
				<ZKDetailGrid >
		            <ZKDetailGrid.Row>
						<ZKDetailGrid.ColLabel>{zkToolsMsg.msgFormatByIntl(intl, 'zk.sys.res.SysResApplicationSystem.code')}:</ZKDetailGrid.ColLabel>
    ...
    componentDidMount() {
        let { location, dispatch, match, mSysResApplicationSystem } = this.props;
		let { params } = match;
		// 第一次进来或地址栏改变了 
...
export default injectIntl(connect(({ mApp, mSysResApplicationSystem, loading }) => ({ mApp, mSysResApplicationSystem, loading }))(CInitSysResApplicationSystemDetail));