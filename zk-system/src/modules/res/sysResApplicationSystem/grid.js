...
const f_getTableColumns = (onEedit, onDetail, onDelete, intl) => {
	...
	return [
		{
			title: zkToolsMsg.msgFormatByIntl(intl, 'zk.sys.res.SysResApplicationSystem.name'),
		...
		{
			title: zkToolsMsg.msgFormatByIntl(intl, 'zk.sys.res.SysResApplicationSystem.code'),
			textAlign: 'center', dataIndex: 'code', key: 'code', width: 100, 
		},
		{
			title: zkToolsMsg.msgFormatByIntl(intl, 'zk.sys.res.SysResApplicationSystem.shortName'),
			textAlign: 'center', dataIndex: 'shortName', key: 'shortName', width: 100, 
			render: (text, record, index) => {
				return zkToolsMsg.getInternationInfo(record.shortName?record.shortName:{}, intl.locale);
			}
		},
		{
...
class CInitSysResApplicationSystemGrid extends React.Component {
	...
	f_changeSelKeys = (selRowKeys, selRows) => {
		this.props.dispatch({ type: "mSysResApplicationSystem/setState", payload: { gridSelKeys: selRowKeys } });
	}
	...
	render() {
		...
		let { intl, mApp, mSysResApplicationSystem, loading } = this.props;

		let tableColumns = f_getTableColumns(this.f_edit, this.f_detail, this.f_delete, intl);

		let gridLoading = loading.effects['mSysResApplicationSystem/findSysResApplicationSystems'];
		...
		return (
			<ZKScrollTable loading = { gridLoading }
				...
				rowSelection = {{
					...
					selectedRowKeys: mSysResApplicationSystem.gridSelKeys||[], 
					columnWidth: '32px'
				}}
				rowKey = "pkId"
			...
			>
				<ZKOptRow>
					<ZKOptRow.OptGroup>
						...
						<ZKOptRow.OptGroup.OptItem onClick={(e) => {
							// 删除
							this.f_delete(mSysResApplicationSystem.gridSelKeys, true);
...
export default CInitSysResApplicationSystemGrid;