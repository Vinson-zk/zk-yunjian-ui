...
const f_getTableColumns = (onEdit, onDetail, onDelete, intl) => {
	...
	return [
		// {
		// 	title: zkToolsMsg.msgFormatByIntl(intl, 'zk.sys.res.SysResDict.parentName'),
		// 	textAlign: 'center', dataIndex: 'parentId', key: 'parentId', width: 100
		// },
		{
			title: zkToolsMsg.msgFormatByIntl(intl, 'zk.sys.res.SysResDict.typeCode'),
		...
		{
			title: zkToolsMsg.msgFormatByIntl(intl, 'zk.sys.res.SysResDict.dictCode'),
			textAlign: 'center', dataIndex: 'dictCode', key: 'dictCode', width: 100, 
		},
		{
			title: zkToolsMsg.msgFormatByIntl(intl, 'zk.sys.res.SysResDict.dictName'),
			textAlign: 'center', dataIndex: 'dictName', key: 'dictName', width: 100, 
			render: (text, record, index) => {
				return zkToolsMsg.getInternationInfo(record.dictName?record.dictName:{}, intl.locale);
			}
		},
		{
			title: zkToolsMsg.msgFormatByIntl(intl, 'zk.sys.res.SysResDict.dictDesc'),
			textAlign: 'center', dataIndex: 'dictDesc', key: 'dictDesc', width: 100, 
			render: (text, record, index) => {
				return zkToolsMsg.getInternationInfo(record.dictDesc?record.dictDesc:{}, intl.locale);
			}
		},
		{
...
class CInitSysResDictGrid extends React.Component {
	...
	f_changeSelKeys = (selRowKeys, selRows) => {
		this.props.dispatch({ type: "mSysResDict/setState", payload: { gridSelKeys: selRowKeys } });
	}
	...
	/** 返回 JSX 元素 */
	render() {
		let { intl, mApp, mSysResDict, loading } = this.props;
		...
		return (
			<ZKScrollTable loading = { gridLoading }
				...
				rowSelection = {{
					...
					selectedRowKeys: mSysResDict.gridSelKeys||[], 
					columnWidth: '32px'
				}}
				rowKey = "pkId"
			...
			>
				<ZKOptRow>
					<ZKOptRow.OptGroup>
						...
						<ZKOptRow.OptGroup.OptItem onClick={(e) => {
							// 删除
							this.f_delete(mSysResDict.gridSelKeys, true);
						}} >
...
export default CInitSysResDictGrid;