...
class CInitSysResDictDetail extends Component {

	// 1、构造函数
	constructor(props) {
		super(props);
		this.state = {
			// sysResDictType: undefined,
		};
	}
	...
	render() {
		...
		return (optEntity != undefined && mSysResDict.pathname == location.pathname) && (
			<ZKSpin spinning={spinning === true} >
				<ZKDetailGrid >
					<ZKDetailGrid.Row>
						<ZKDetailGrid.ColLabel>{zkToolsMsg.msgFormatByIntl(intl, 'zk.sys.res.SysResDict.parentName')}:</ZKDetailGrid.ColLabel>
						<ZKDetailGrid.ColValue>
							{ optEntity.parent?(optEntity.parent.name?zkToolsMsg.getInternationInfo(optEntity.parent.name):optEntity.parent.pkId):zkToolsMsg.msgFormatByIntl(intl, 'zk.sys.res.SysResDict._top') }
						</ZKDetailGrid.ColValue>
					</ZKDetailGrid.Row>
		            <ZKDetailGrid.Row>
						<ZKDetailGrid.ColLabel>{zkToolsMsg.msgFormatByIntl(intl, 'zk.sys.res.SysResDict.typeCode')}:</ZKDetailGrid.ColLabel>
						<ZKDetailGrid.ColValue>
						...
						<ZKDetailGrid.ColLabel>{zkToolsMsg.msgFormatByIntl(intl, 'zk.sys.res.SysResDict.dictCode')}:</ZKDetailGrid.ColLabel>
						<ZKDetailGrid.ColValue>
							{optEntity.dictCode}
						</ZKDetailGrid.ColValue>
					</ZKDetailGrid.Row>       
...
export default injectIntl(connect(({ mApp, mSysResDict, loading }) => ({ mApp, mSysResDict, loading }))(CInitSysResDictDetail));