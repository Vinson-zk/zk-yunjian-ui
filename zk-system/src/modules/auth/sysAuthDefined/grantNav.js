...
* @Last Modified time: 2022-05-09 17:26:46
* 
* 
* 
*/
...
class CInitGrantNav extends Component {
	constructor(props) {
        super(props);
        this.state = {
        	isInit: false,
        ...
        try{
	        ...
	        f_doingSearch(this.props.url, params).done(res=>{
		    	if (res.code == 'zk.0') {
	            	...
	            	throw new Error("[>_<:20220504-1538-002] request url:[" + this.props.url + "] exception: " + res.code);
    		...
    		try{
		        ...
		        f_doingSearch(this.props.urlOwnerIds, params).done(res=>{
			    	if (res.code == 'zk.0') {
		            	...
		            	throw new Error("[>_<:20220504-1538-003] request url:[" + this.props.urlOwnerIds + "] exception: " + res.code);
    ...
    f_handleOk = ()=>{
		...
		dispatch({ 
		  type: 'mSysAuthDefined/grantNavs', 
		  authId: this.props.targetId,
		  navs: this.state.selectedObjs, 
		  callback: dataObj=>{
		    this.f_close();
		  } 
		});
    ...
    f_close = ()=>{
    	...
    	if(this.props.onShowModal instanceof Function){
    		this.props.onShowModal.call(this, "grantNavModal", false, {});
    	}
    };
	...
	render(){
		...
		let spinning = loading.effects['mSysAuthDefined/grantNavs'];
		let gridSpinning = this.state.spinningListDatas || this.state.spinningOwnerIds;

	    return (
			<ZKModal title={`${zkToolsMsg.msgFormatByIntl(intl, 'zk.sys.auth.SysAuthDefined.opt.grantNav')}[${descName}]`} visible={isShow}
			...
			>
				<ZKSpin spinning={spinning === true} >
		            ...
		            >    
		                <ZKSearchItem name = "searchValue" label = {`${zkToolsMsg.msgFormatByIntl(intl, 'zk.system.nav.name')}/${zkToolsMsg.msgFormatByIntl(intl, 'zk.system.nav.code')}`} >
	  	...
	  	if(nextProps.isShow && this.state.isInit == false){
	  		...
	  		if(nextProps.targetId){
		    	...
		    	this.f_onSearch();
	  			...
	  			throw new Error("[>_<:2020504-1658-001] 分配导航栏目时，当前操作权限 [targetId] 必传入", nextProps.targetId);
	  		}
	  	}
	  	return true;
	  }
...
// 定义属性默认值
CInitGrantNav.defaultProps = {
	url: `/${globalAppConfig.apiPrefixSys}/res/nav/sysNavs`,
	urlOwnerIds: `/${globalAppConfig.apiPrefixSys}/auth/sysAuthNav/findNavIdsByAuthId`,
	isShow: false,
	targetId: undefined, // 操作对象ID
	descName: "", // 操作对象说明名称
}

export default injectIntl(connect(({ mApp, loading }) => ({ mApp, loading }))(CInitGrantNav));
