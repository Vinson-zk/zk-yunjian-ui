...
class CInitSysOrgDeptGrid extends React.Component {
	...
	f_delete = (keys, isConfirm) => {
		...
		let f_executeDelete = (pkIds) => {
            _this.props.dispatch({
                type: "mSysOrgDept/delSysOrgDept", payload: { pkId: pkIds },
                callback: () => {
                    _this.props.dispatch({ 
                    	type: 'mSysOrgDept/findSysOrgDeptsTree', 
                    	filter: _this.props.mSysOrgDept.filter, 
                    	pagination: _this.props.mSysOrgDept.pagination, 
                    	callback: e => { } })
                }
            });
		};
	...
	f_changeSelKeys = (selRowKeys, selRows) => {
		this.props.dispatch({ type: "mSysOrgDept/setState", payload: { gridSelKeys: selRowKeys } });
	}
	...
	f_changeGrid = (pagination, filters, sorter) => {
        ...
        this.props.dispatch({ 
        	type: 'mSysOrgDept/findSysOrgDepts', 
            filter: this.props.mSysOrgDept.filter,
		...
		return (
			<ZKScrollTable loading = { gridLoading }
				...
				rowSelection = {{
					...
					selectedRowKeys: mSysOrgDept.gridSelKeys||[], 
					columnWidth: '32px'
				}}
				rowKey = "pkId"
				...
				pagination = {mSysOrgDept.pagination || {}}
				// pagination = {{position: ['topRight'], ...page}}
                dataSource = {mSysOrgDept.gridData||[]}
			...
			>
				<ZKOptRow>
					<ZKOptRow.OptGroup>
						...
						<ZKOptRow.OptGroup.OptItem onClick={(e) => {
							// 删除
							this.f_delete(mSysOrgDept.gridSelKeys, true);
						}} >
