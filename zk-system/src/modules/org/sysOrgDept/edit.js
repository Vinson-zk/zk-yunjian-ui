...
class CInitSysOrgDeptEdit extends Component {
    ...
    f_save = (values, form, callbackFunc) => {
        ...
        this.props.dispatch({
            type: 'mSysOrgDept/editSysOrgDept', 
            payload: values, 
            callback: (errors) => {
                if(!errors){
    ...
    render() {
        ...
        let spinning = !optEntity || this.state.loading
        	 || loading.effects['mSysOrgDept/editSysOrgDept'] || loading.effects['mSysOrgDept/getSysOrgDept'];
                ...
                >
                    ...
                    <ZKRow><ZKCol span = {24} >
                        <ZKEditForm.Item labelCol = {{span: 5}} wrapperCol = {{span:15}} label = {zkToolsMsg.msgFormatByIntl(intl, 'zk.sys.org.SysOrgDept.companyName')} >
                            { optEntity.company.name?zkToolsMsg.getInternationInfo(optEntity.company.name):"" }
                        </ZKEditForm.Item>
                    </ZKCol></ZKRow>
    ...
    componentDidMount() {
        let { location, match, dispatch, mSysOrgDept, mApp } = this.props;
        let { params } = match;
        if (mSysOrgDept.pathname != location.pathname) {
            ...
            if("_new" == params.pkId){
                if("_top" == params.parentId){
                    // 新增根结点；不需要查父节点信息；
                    dispatch({ type: 'mSysOrgDept/setState', payload: { optEntity:{ company: mApp.user.company } } });
                }else{
                	// 新增子节点；需要查父节点信息；不管父节点存不存在，都从后台查询
                    dispatch({ type: 'mSysOrgDept/getSysOrgDept', payload: { pkId: params.parentId }, isParent: true });
                }
            }else{
                // 编辑；则根据 id 取后台数据；
                dispatch({ type: 'mSysOrgDept/getSysOrgDept', payload: { pkId: params.pkId } });
            }
        }
    }
...
export default injectIntl(connect(({ mApp, mSysOrgDept, loading }) => ({ mApp, mSysOrgDept, loading }))(CInitSysOrgDeptEdit));
