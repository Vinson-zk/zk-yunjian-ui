...
* @Last Modified time: 2024-07-07 17:59:39
*/
...
const model = {
    ...
    effects: { // action
        // 编辑 
        *editSysOrgDept({ payload, callback }, { call }) {
        	if(zkJsUtils.isEmpty(payload.parentId)){
                delete payload.parentId;
            }
            
            let res = yield call(editSysOrgDept, payload);
            let f = errors=>{
        ...
        *delSysOrgDept({ payload, callback }, { call }) {
        ...
        // 查询 详情
        *getSysOrgDept({ payload, isParent = false }, { call, put }) {
            ...
            if (res.ok) {
            	if(isParent){
                    ...
                    optEntity.company = res.data.company;
         ...
         */
        *findSysOrgDeptsTree({ filter, pagination, sorter, callback }, { call, put, select }) {
            let params = zkToolsUtils.convertSortParam(filter, sorter); 
            if(pagination){
                params = { ...params, ...zkToolsUtils.convertPageParam(pagination) };
            }
            let res = yield call(findSysOrgDeptsTree, params);
            let restState = {}
            if (res.ok) {
                restState = {
                    "filter": params,
