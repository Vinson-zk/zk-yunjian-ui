...
* @Last Modified time: 2024-07-07 10:51:19
* 
* 
* 
*/
...
class CInitGrantAuth extends Component {
	constructor(props) {
        ...
        this.state = {
        	spinning: false, 
        	toTargetId: undefined, // 当前被分配权限的目标对象ID
            selectedKeys:[],
            authDatas: [],
            filter: {},
            pagination:{
    ...
    // 查询公司可分配的授权
    f_findAuthDatas = (toTargetId, params={}, sorter, pagination={})=>{
    	try{
        	this.setState({spinning: true});
        	params = zkToolsUtils.convertSortParam(params, sorter); 
	        params = { ...params, ...zkToolsUtils.convertPageParam(pagination)};
	        // 格式化请求参数，由引用授权的组件传入格式请求参数
	        if(zkJsUtils.assertObjType(this.props.formatParamsFunc, Function)){
	        	params = this.props.formatParamsFunc.call(this, toTargetId, params);
	        }
	        f_doingSearch(this.props.url, params).done(res=>{
        		// console.log("[^_^:20220504-1739-003] CInitGrantAuth.f_findAuthDatas: ", res);
		    	if (res.ok) {
		    		let state = {
		    			"filter": params,
		    			"selectedKeys": [],
	...
	render(){
	    ...
	    return (
			<ZKModal title={`${title}[${descName}]`} open={isShow}
				...
				<ZKSpin spinning={saveSpinning === true} >
					<ZKSearchRow 
		                ...
		                searchFunc={values => {
		                	...
		                	this.f_findAuthDatas(this.state.toTargetId, values, null, pagination);
		                }}
		            >    
				    ...
				    <ZKScrollTable loading = { this.state.spinning }
		                ...
		                onChange = { (pagination, filters, sorter, extra)=>{ // 列表改变
        					...
        					this.f_findAuthDatas(this.state.toTargetId, this.state.filters, sorter, pagination);
		                }}
	...
	// 3、更新时调用，此方法仅作为性能优化存在。不要依赖它来“防止”渲染
	  shouldComponentUpdate(nextProps, nextState){
	  	...
	  	if(nextProps.isShow){
			if(nextProps.toTargetId != this.state.toTargetId){
				// console.log("[^_^:20220504-1739-002] CInitGrantAuthToCompany.shouldComponentUpdate:", nextState);
				nextState.toTargetId = nextProps.toTargetId;
				if(nextProps.toTargetId){
					this.f_findAuthDatas(nextProps.toTargetId, this.state.filters, null, this.state.pagination);
				}else{
...
CInitGrantAuth.propTypes = {
	...
	formatParamsFunc: PropTypes.func, // 格式化请求参数
