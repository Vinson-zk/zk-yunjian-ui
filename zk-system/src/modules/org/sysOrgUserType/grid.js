...
const f_getTableColumns = (onEedit, onDetail, onDelete, intl, lang, onShowGrantAuthModal) => {
	...
	return [
		{
			title: zkToolsMsg.msgFormatByIntl(intl, 'zk.sys.org.SysOrgUserType.code'),
			textAlign: 'center', dataIndex: 'code', key: 'code', width: 100, 
		},
		{
			title: zkToolsMsg.msgFormatByIntl(intl, 'zk.sys.org.SysOrgUserType.name'),
		...
		{
			title: zkToolsMsg.msgFormatByIntl(intl, 'zk.sys.org.SysOrgUserType.status'),
			textAlign: 'center', dataIndex: 'status', key: 'status', width: 100, 
			render: (text, record, index) => {
				return zkToolsMsg.msgFormatByIntl(intl, 'zk.sys.org.SysOrgUserType.status.' + text);
			}
		},
		{
			title: zkToolsMsg.msgFormatByIntl(intl, 'zk.sys.org.SysOrgUserType.groupCode'),
			textAlign: 'center', dataIndex: 'groupCode', key: 'groupCode', width: 100, 
		},
		{
			title: zkToolsMsg.msgFormatByIntl(intl, 'zk.sys.org.SysOrgUserType.companyCode'),
			textAlign: 'center', dataIndex: 'companyCode', key: 'companyCode', width: 100, 
		},
		{
			title: zkToolsMsg.msgFormatByIntl(intl, 'zk.sys.org.SysOrgUserType.sourceCode'),
			textAlign: 'center', dataIndex: 'sourceCode', key: 'sourceCode', width: 100, 
		},
		{
			title: zkToolsMsg.msgFormatByIntl(intl, 'zk.sys.org.SysOrgUserType.sourceId'),
...
class CInitSysOrgUserTypeGrid extends React.Component {
	...
	f_delete = (keys, isConfirm) => {
		...
		let f_executeDelete = (pkIds) => {
            _this.props.dispatch({
                type: "mSysOrgUserType/delSysOrgUserType", payload: { pkId: pkIds },
                callback: () => {
                    _this.props.dispatch({ type: 'mSysOrgUserType/findSysOrgUserTypes', filter: _this.props.mSysOrgUserType.filter, pagination: _this.props.mSysOrgUserType.pagination, callback: e => { } })
                }
            });
		};
	...
	f_changeSelKeys = (selRowKeys, selRows) => {
		this.props.dispatch({ type: "mSysOrgUserType/setState", payload: { gridSelKeys: selRowKeys } });
	}
	...
	f_changeGrid = (pagination, filters, sorter) => {
        ...
        this.props.dispatch({ 
            ...
            filter: this.props.mSysOrgUserType.filter,
	...
	render() {
		...
		let { intl, mApp, mSysOrgUserType, loading, onShowGrantAuthModal } = this.props;
        let lang = mApp.lang?mApp.lang:zkToolsMsg.getLocale();

		let tableColumns = f_getTableColumns(this.f_edit, this.f_detail, this.f_delete, intl, lang, onShowGrantAuthModal);

		let gridLoading = loading.effects['mSysOrgUserType/findSysOrgUserTypes'];
		...
		return (
			<ZKScrollTable loading = { gridLoading }
				...
				rowSelection = {{
					...
					selectedRowKeys: mSysOrgUserType.gridSelKeys||[], 
					columnWidth: '32px'
				}}
				rowKey = "pkId"
				...
				pagination = {mSysOrgUserType.pagination||{}}
				// pagination = {{position: ['topRight'], ...page}}
                dataSource = {mSysOrgUserType.gridData||[]}
			...
			>
				<ZKOptRow>
					<ZKOptRow.OptGroup>
						...
						<ZKOptRow.OptGroup.OptItem onClick={(e) => {
							// 删除
							this.f_delete(mSysOrgUserType.gridSelKeys, true);
...
export default CInitSysOrgUserTypeGrid;