...
const f_getTableColumns = (onEedit, onDetail, onDelete, intl, lang, onShowGrantAuthModal) => {
	...
	return [
		{
			title: zkToolsMsg.msgFormatByIntl(intl, 'zk.sys.org.SysOrgRank.code'),
			textAlign: 'center', dataIndex: 'code', key: 'code', width: 100, 
		},
		{
			title: zkToolsMsg.msgFormatByIntl(intl, 'zk.sys.org.SysOrgRank.name'),
		...
		{
			title: zkToolsMsg.msgFormatByIntl(intl, 'zk.sys.org.SysOrgRank.status'),
			textAlign: 'center', dataIndex: 'status', key: 'status', width: 100, 
			render: (text, record, index) => {
				return zkToolsMsg.msgFormatByIntl(intl, 'zk.sys.org.SysOrgRank.status.' + text);
			}
		},
		{
			title: zkToolsMsg.msgFormatByIntl(intl, 'zk.sys.org.SysOrgRank.groupCode'),
			textAlign: 'center', dataIndex: 'groupCode', key: 'groupCode', width: 100, 
		},
		{
			title: zkToolsMsg.msgFormatByIntl(intl, 'zk.sys.org.SysOrgRank.companyCode'),
			textAlign: 'center', dataIndex: 'companyCode', key: 'companyCode', width: 100
		},
		{
			title: zkToolsMsg.msgFormatByIntl(intl, 'zk.sys.org.SysOrgRank.sourceCode'),
			textAlign: 'center', dataIndex: 'sourceCode', key: 'sourceCode', width: 100, 
		},
		{
			title: zkToolsMsg.msgFormatByIntl(intl, 'zk.sys.org.SysOrgRank.sourceId'),
...
class CInitSysOrgRankGrid extends React.Component {
	...
	f_delete = (keys, isConfirm) => {
		...
		let f_executeDelete = (pkIds) => {
            _this.props.dispatch({
                type: "mSysOrgRank/delSysOrgRank", payload: { pkId: pkIds },
                callback: () => {
                    _this.props.dispatch({ type: 'mSysOrgRank/findSysOrgRanks', filter: _this.props.mSysOrgRank.filter, pagination: _this.props.mSysOrgRank.pagination, callback: e => { } })
                }
            });
		};
	...
	f_changeSelKeys = (selRowKeys, selRows) => {
		this.props.dispatch({ type: "mSysOrgRank/setState", payload: { gridSelKeys: selRowKeys } });
	}
	...
	f_changeGrid = (pagination, filters, sorter) => {
        ...
        this.props.dispatch({ 
            ...
            filter: this.props.mSysOrgRank.filter,
	...
	render() {
		...
		let { intl, mApp, mSysOrgRank, loading, onShowGrantAuthModal } = this.props;
        let lang = mApp.lang?mApp.lang:zkToolsMsg.getLocale();

		let tableColumns = f_getTableColumns(this.f_edit, this.f_detail, this.f_delete, intl, lang, onShowGrantAuthModal);

		let gridLoading = loading.effects['mSysOrgRank/findSysOrgRanks'];
		...
		return (
			<ZKScrollTable loading = { gridLoading }
				...
				rowSelection = {{
					...
					selectedRowKeys: mSysOrgRank.gridSelKeys||[], 
					columnWidth: '32px'
				}}
				rowKey = "pkId"
				...
				pagination = {mSysOrgRank.pagination||{}}
			...
			>
				<ZKOptRow>
					<ZKOptRow.OptGroup>
						...
						<ZKOptRow.OptGroup.OptItem onClick={(e) => {
							// 删除
							this.f_delete(mSysOrgRank.gridSelKeys, true);
...
export default CInitSysOrgRankGrid;