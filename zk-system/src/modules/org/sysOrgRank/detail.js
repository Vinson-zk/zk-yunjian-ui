...
class CInitSysOrgRankDetail extends Component {
  ...
  render() {
    ...
    return (optEntity != undefined && mSysOrgRank.pathname == location.pathname) && (
      <ZKSpin spinning={spinning === true} >
        <ZKDetailGrid >
          ...
          <ZKDivider />
          <ZKDetailGrid.Row>
            <ZKDetailGrid.ColLabel>{zkToolsMsg.msgFormatByIntl(intl, 'zk.sys.org.SysOrgRank.code')}:</ZKDetailGrid.ColLabel>
            <ZKDetailGrid.ColValue>
              {optEntity.code}
            </ZKDetailGrid.ColValue> 
            <ZKDetailGrid.ColLabel>{zkToolsMsg.msgFormatByIntl(intl, 'zk.sys.org.SysOrgRank.status')}:</ZKDetailGrid.ColLabel>
            <ZKDetailGrid.ColValue>
              {zkToolsMsg.msgFormatByIntl(intl, 'zk.sys.org.SysOrgRank.status.' + optEntity.status)}</ZKDetailGrid.ColValue>
          </ZKDetailGrid.Row>
          <ZKDetailGrid.Row>
  ...
  componentDidMount() {
    let { location, dispatch, match, mSysOrgRank } = this.props;
    let { params } = match;
...
export default injectIntl(connect(({ mApp, mSysOrgRank, loading }) => ({ mApp, mSysOrgRank, loading }))(CInitSysOrgRankDetail));