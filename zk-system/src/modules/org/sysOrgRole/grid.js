...
const f_getTableColumns = (onEedit, onDetail, onDelete, intl, lang, onShowGrantAuthModal) => {
	...
	return [
		...
		{
			title: zkToolsMsg.msgFormatByIntl(intl, 'zk.sys.org.SysOrgRole.groupCode'),
			textAlign: 'center', dataIndex: 'groupCode', key: 'groupCode', width: 100, 
		},
		{
			title: zkToolsMsg.msgFormatByIntl(intl, 'zk.sys.org.SysOrgRole.companyCode'),
...
class CInitSysOrgRoleGrid extends React.Component {
	...
	f_delete = (keys, isConfirm) => {
		...
		let f_executeDelete = (pkIds) => {
            _this.props.dispatch({
                type: "mSysOrgRole/delSysOrgRole", payload: { pkId: pkIds },
                callback: () => {
                    _this.props.dispatch({ type: 'mSysOrgRole/findSysOrgRoles', filter: _this.props.mSysOrgRole.filter, pagination: _this.props.mSysOrgRole.pagination, callback: e => { } })
                }
            });
		};
	...
	f_changeSelKeys = (selRowKeys, selRows) => {
		this.props.dispatch({ type: "mSysOrgRole/setState", payload: { gridSelKeys: selRowKeys } });
	}
	...
	f_changeGrid = (pagination, filters, sorter) => {
        ...
        this.props.dispatch({ 
            ...
            filter: this.props.mSysOrgRole.filter,
		...
		return (
			<ZKScrollTable loading = { gridLoading }
				...
				rowSelection = {{
					...
					selectedRowKeys: mSysOrgRole.gridSelKeys||[], 
					columnWidth: '32px'
				}}
				rowKey = "pkId"
				...
				pagination = {mSysOrgRole.pagination||{}}
				// pagination = {{position: ['topRight'], ...page}}
                dataSource = {mSysOrgRole.gridData||[]}
			...
			>
				<ZKOptRow>
					<ZKOptRow.OptGroup>
						...
						<ZKOptRow.OptGroup.OptItem onClick={(e) => {
							// 删除
							this.f_delete(mSysOrgRole.gridSelKeys, true);
...
export default CInitSysOrgRoleGrid;