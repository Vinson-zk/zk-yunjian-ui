...
class CInitSysOrgCompanySearch extends React.Component {
    ...
    f_search = filter=>{
        let { dispatch, mSysOrgCompany } = this.props;
        if(!filter){
            filter = mSysOrgCompany.initFilter;
        }
        dispatch({ 
            type: "mSysOrgCompany/findSysOrgChildCompanysTree", 
            filter: {...mSysOrgCompany.filter, ...filter}, 
            callback: e => { } 
        });
    }
...
*/
        return (
            <ZKSearchRow 
                initialValues={mSysOrgCompany.initFilter} 
                filter={mSysOrgCompany.filter||mSysOrgCompany.initFilter} 
                resetFunc={values => {}}
                searchFunc={values => {
