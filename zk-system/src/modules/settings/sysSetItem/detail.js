...
class CInitSysSetItemDetail extends Component {
	...
	render() {
		...
		return (optEntity != undefined && mSysSetItem.pathname == location.pathname) && (
			<ZKSpin spinning={spinning === true} >
				<ZKDetailGrid >
		            <ZKDetailGrid.Row>
						<ZKDetailGrid.ColLabel>{zkToolsMsg.msgFormatByIntl(intl, 'zk.sys.settings.SysSetItem.type')}:</ZKDetailGrid.ColLabel>
						<ZKDetailGrid.ColValue>
							{zkToolsMsg.msgFormatByIntl(intl, `zk.sys.settings.SysSetItem.type.${optEntity.type}`)}
						</ZKDetailGrid.ColValue>
					</ZKDetailGrid.Row>     
					{optEntity.type == 2?(
                        <React.Fragment>
                            <ZKDetailGrid.Row>
								<ZKDetailGrid.ColLabel>{zkToolsMsg.msgFormatByIntl(intl, 'zk.sys.settings.SysSetItem.groupCode')}:</ZKDetailGrid.ColLabel>
								<ZKDetailGrid.ColValue>
									{optEntity.groupCode}
								</ZKDetailGrid.ColValue>
								<ZKDetailGrid.ColLabel>{zkToolsMsg.msgFormatByIntl(intl, 'zk.sys.settings.SysSetItem.compamyCode')}:</ZKDetailGrid.ColLabel>
								<ZKDetailGrid.ColValue>
		            ...
		            <ZKDetailGrid.Row>
						<ZKDetailGrid.ColLabel>{zkToolsMsg.msgFormatByIntl(intl, 'zk.sys.settings.SysSetItem.value')}:</ZKDetailGrid.ColLabel>
						<ZKDetailGrid.ColValue>
							{optEntity.value}
						</ZKDetailGrid.ColValue>
					</ZKDetailGrid.Row>             
    ...
    componentDidMount() {
        let { location, dispatch, match, mSysSetItem } = this.props;
		let { params } = match;
		// 第一次进来或地址栏改变了 
		if (mSysSetItem.pathname != location.pathname ) {
			dispatch({ type: 'mSysSetItem/setState', payload: { pathname: location.pathname, optEntity: undefined } });
...
export default injectIntl(connect(({ mApp, mSysSetItem, loading }) => ({ mApp, mSysSetItem, loading }))(CInitSysSetItemDetail));
