 ...
 * @Last Modified time: 2024-06-24 17:44:02
 */

import { editSysSetItem, delSysSetItem, getSysSetItem, findSysSetItems } from './service';

import { zkTools } from 'zkFramework';
const { zkToolsUtils, zkToolsMsg } = zkTools;

const model = {
    namespace: 'mSysSetItem',
    state: {
    ...
    effects: { // action
        // 编辑 
        *editSysSetItem({ payload, callback }, { call }) {
            let res = yield call(editSysSetItem, payload);
            let f = errors=>{
        ...
        // 删除
        *delSysSetItem({ payload, callback }, { call }) {
            let res = yield call(delSysSetItem, payload);
            if(res.ok){
                zkToolsMsg.alertMsg(null, null, {type:"success", msg:res.msg});
            }
            if (zkJsUtils.assertObjType(callback, Function)) {
        ...
        // 查询 详情
        *getSysSetItem({ payload }, { call, put }) {
            let res = yield call(getSysSetItem, payload);
            if (res.ok) {
