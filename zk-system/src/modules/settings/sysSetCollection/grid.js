...
const f_getTableColumns = (onEedit, onDetail, onDelete, intl, lang) => {
	...
	return [
		{
			title: zkToolsMsg.msgFormatByIntl(intl, 'zk.sys.settings.SysSetCollection.name'),
		...
		{
			title: zkToolsMsg.msgFormatByIntl(intl, 'zk.sys.settings.SysSetCollection.code'),
			textAlign: 'center', dataIndex: 'code', key: 'code', width: 100, 
		},
		{
			title: zkToolsMsg.msgFormatByIntl(intl, 'zk.sys.settings.SysSetCollection.setDesc'),
		...
		{
			title: zkToolsMsg.msgFormatByIntl(intl, 'zk.sys.settings.SysSetCollection.groupCode'),
			textAlign: 'center', dataIndex: 'groupCode', key: 'groupCode', width: 100, 
		},
		{
			title: zkToolsMsg.msgFormatByIntl(intl, 'zk.sys.settings.SysSetCollection.compamyCode'),
...
class CInitSysSetCollectionGrid extends React.Component {
	...
	f_delete = (keys, isConfirm) => {
		...
		let f_executeDelete = (pkIds) => {
            _this.props.dispatch({
                type: "mSysSetCollection/delSysSetCollection", payload: { pkId: pkIds },
                callback: () => {
                    _this.props.dispatch({ type: 'mSysSetCollection/findSysSetCollections', filter: _this.props.mSysSetCollection.filter, pagination: mSysSetCollection.pagination, callback: e => { } })
                }
            });
		};
	...
	f_changeSelKeys = (selRowKeys, selRows) => {
		this.props.dispatch({ type: "mSysSetCollection/setState", payload: { gridSelKeys: selRowKeys } });
	}
	...
	f_changeGrid = (pagination, filters, sorter) => {
        ...
        this.props.dispatch({ 
            ...
            filter: this.props.mSysSetCollection.filter,
	...
	render() {
		...
		let { intl, mApp, mSysSetCollection, loading } = this.props;
        let lang = mApp.lang?mApp.lang:zkToolsMsg.getLocale();

		let tableColumns = f_getTableColumns(this.f_edit, this.f_detail, this.f_delete, intl, lang);

		let gridLoading = loading.effects['mSysSetCollection/findSysSetCollections'];
		...
		return (
			<ZKScrollTable loading = { gridLoading }
				...
				rowSelection = {{
					...
					selectedRowKeys: mSysSetCollection.gridSelKeys||[], 
					columnWidth: '32px'
				}}
				rowKey = "pkId"
				...
				pagination = {mSysSetCollection.page||{}}
				// pagination = {{position: ['topRight'], ...page}}
                dataSource = {mSysSetCollection.gridData||[]}
			...
			>
				<ZKOptRow>
					<ZKOptRow.OptGroup>
						...
						<ZKOptRow.OptGroup.OptItem onClick={(e) => {
							// 删除
							this.f_delete(mSysSetCollection.gridSelKeys, true);
...
export default CInitSysSetCollectionGrid;