 ...
 * @Last Modified time: 2024-06-24 17:44:58
 */

import { editPayGetBusinessType, delPayGetBusinessType, getPayGetBusinessType, findPayGetBusinessTypes } from './service';

import { zkTools } from 'zkFramework';
const { zkToolsUtils, zkToolsMsg } = zkTools;

const model = {
    namespace: 'mPayGetBusinessType',
    state: {
    ...
    effects: { // action
        // 编辑 
        *editPayGetBusinessType({ payload, callback }, { call }) {
            let res = yield call(editPayGetBusinessType, payload);
            let f = errors=>{
        ...
        // 删除
        *delPayGetBusinessType({ payload, callback }, { call }) {
            let res = yield call(delPayGetBusinessType, payload);
            if(res.ok){
                zkToolsMsg.alertMsg(null, null, {type:"success", msg:res.msg});
            }
            if (zkJsUtils.assertObjType(callback, Function)) {
        ...
        // 查询 详情
        *getPayGetBusinessType({ payload }, { call, put }) {
            let res = yield call(getPayGetBusinessType, payload);
            if (res.ok) {
         ...
         */
        *findPayGetBusinessTypes({ filter, pagination, sorter, callback }, { call, put, select }) {
            let params = zkToolsUtils.convertSortParam(filter, sorter); 
            if(pagination){
                params = { ...params, ...zkToolsUtils.convertPageParam(pagination) };
            }
            let res = yield call(findPayGetBusinessTypes, params);
            let restState = {}
            if (res.ok) {
                restState = {
                    "filter": params,
