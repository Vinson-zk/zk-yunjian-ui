 ...
 * @Last Modified time: 2024-06-24 17:48:46
 */

import { editPayMerchant, delPayMerchant, getPayMerchant, findPayMerchants } from './service';

import { zkTools } from 'zkFramework';
const { zkToolsUtils, zkToolsMsg } = zkTools;

const model = {
    namespace: 'mPayMerchant',
    state: {
    ...
    effects: { // action
        // 编辑 
        *editPayMerchant({ payload, callback }, { call }) {
            let res = yield call(editPayMerchant, payload);
            let f = errors=>{
        ...
        // 删除
        *delPayMerchant({ payload, callback }, { call }) {
            let res = yield call(delPayMerchant, payload);
            if(res.ok){
                zkToolsMsg.alertMsg(null, null, {type:"success", msg:res.msg});
            }
            if (zkJsUtils.assertObjType(callback, Function)) {
        ...
        // 查询 详情
        *getPayMerchant({ payload }, { call, put }) {
            let res = yield call(getPayMerchant, payload);
            if (res.ok) {
         ...
         */
        *findPayMerchants({ filter, pagination, sorter, callback }, { call, put, select }) {
            let params = zkToolsUtils.convertSortParam(filter, sorter); 
            if(pagination){
                params = { ...params, ...zkToolsUtils.convertPageParam(pagination) };
            }
            let res = yield call(findPayMerchants, params);
            let restState = {}
            if (res.ok) {
                restState = {
                    "filter": params,
