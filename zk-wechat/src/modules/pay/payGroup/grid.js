...
const f_getTableColumns = (onEedit, onDetail, onDelete, intl, lang) => {
	...
	return [
				{
			title: zkToolsMsg.msgFormatByIntl(intl, 'zk.wechat.pay.PayGroup.wxAppId'),
			textAlign: 'center', dataIndex: 'wxAppId', key: 'wxAppId', width: 100, 
		},
		{
			title: zkToolsMsg.msgFormatByIntl(intl, 'zk.wechat.pay.PayGroup.wxMchid'),
			textAlign: 'center', dataIndex: 'wxMchid', key: 'wxMchid', width: 100, 
		},
		{
			title: zkToolsMsg.msgFormatByIntl(intl, 'zk.wechat.pay.PayGroup.code'),
			textAlign: 'center', dataIndex: 'code', key: 'code', width: 100, 
		},
		{
			title: zkToolsMsg.msgFormatByIntl(intl, 'zk.wechat.pay.PayGroup.name'),
		...
		{
			title: zkToolsMsg.msgFormatByIntl(intl, 'zk.wechat.pay.PayGroup.status'),
		...
		{
			title: zkToolsMsg.msgFormatByIntl(intl, 'zk.wechat.pay.PayGroup.startDate'),
			textAlign: 'center', dataIndex: 'startDate', key: 'startDate', width: 100, 
		},
		{
			title: zkToolsMsg.msgFormatByIntl(intl, 'zk.wechat.pay.PayGroup.endDate'),
...
class CInitPayGroupGrid extends React.Component {
	...
	f_changeSelKeys = (selRowKeys, selRows) => {
		this.props.dispatch({ type: "mPayGroup/setState", payload: { gridSelKeys: selRowKeys } });
	}
	...
	render() {
		...
		let { intl, mApp, mPayGroup, loading } = this.props;
        let lang = mApp.lang?mApp.lang:zkToolsMsg.getLocale();

		let tableColumns = f_getTableColumns(this.f_edit, this.f_detail, this.f_delete, intl, lang);

		let gridLoading = loading.effects['mPayGroup/findPayGroups'];
		...
		return (
			<ZKScrollTable loading = { gridLoading }
				...
				rowSelection = {{
					...
					selectedRowKeys: mPayGroup.gridSelKeys||[], 
					columnWidth: '32px'
				}}
				rowKey = "pkId"
...
export default CInitPayGroupGrid;