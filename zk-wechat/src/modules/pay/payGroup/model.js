 ...
 * @Last Modified time: 2024-06-24 17:48:34
 */

import { editPayGroup, delPayGroup, getPayGroup, findPayGroups } from './service';
...
const model = {
    ...
    effects: { // action
        // 编辑 
        *editPayGroup({ payload, callback }, { call }) {
            let res = yield call(editPayGroup, payload);
            let f = errors=>{
        ...
        // 删除
        *delPayGroup({ payload, callback }, { call }) {
            let res = yield call(delPayGroup, payload);
            if(res.ok){
                zkToolsMsg.alertMsg(null, null, {type:"success", msg:res.msg});
            }
            if (zkJsUtils.assertObjType(callback, Function)) {
        ...
        // 查询 详情
        *getPayGroup({ payload }, { call, put }) {
            let res = yield call(getPayGroup, payload);
            if (res.ok) {
