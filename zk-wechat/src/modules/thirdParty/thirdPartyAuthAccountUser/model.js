...
import { editThirdPartyAuthAccountUser, delThirdPartyAuthAccountUser, getThirdPartyAuthAccountUser, findThirdPartyAuthAccountUsers } from './service';

import { zkTools } from 'zkFramework';
const { zkToolsUtils, zkToolsMsg } = zkTools;

const model = {
    namespace: 'mThirdPartyAuthAccountUser',
    state: {
    ...
    effects: { // action
        // 编辑 
        *editThirdPartyAuthAccountUser({ payload, callback }, { call }) {
            let res = yield call(editThirdPartyAuthAccountUser, payload);
            let f = errors=>{
        ...
        // 删除
        *delThirdPartyAuthAccountUser({ payload, callback }, { call }) {
            let res = yield call(delThirdPartyAuthAccountUser, payload);
            if(res.code == "zk.0"){
                zkToolsMsg.alertMsg(null, null, {type:"success", msg:res.msg});
            }
            if (callback instanceof Function) {
        ...
        // 查询 详情
        *getThirdPartyAuthAccountUser({ payload }, { call, put }) {
            let res = yield call(getThirdPartyAuthAccountUser, payload);
            if (res.code == 'zk.0') {
         ...
         */
        *findThirdPartyAuthAccountUsers({ filter, page, sorter, callback }, { call, put, select }) {
            ...
            let res = yield call(findThirdPartyAuthAccountUsers, params);
            let restState = {}
            if (res.code == 'zk.0') {
                restState = {
                    "filter": params,
