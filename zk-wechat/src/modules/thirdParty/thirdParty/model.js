 ...
 * @Last Modified time: 2024-06-24 17:49:09
 */

import { editThirdParty, delThirdParty, getThirdParty, findThirdPartys } from './service';

import { zkTools } from 'zkFramework';
const { zkToolsUtils, zkToolsMsg } = zkTools;

const model = {
    namespace: 'mThirdParty',
    state: {
    ...
    effects: { // action
        // 编辑 
        *editThirdParty({ payload, callback }, { call }) {
            let res = yield call(editThirdParty, payload);
            let f = errors=>{
        ...
        // 删除
        *delThirdParty({ payload, callback }, { call }) {
            let res = yield call(delThirdParty, payload);
            if(res.ok){
                zkToolsMsg.alertMsg(null, null, {type:"success", msg:res.msg});
            }
            if (zkJsUtils.assertObjType(callback, Function)) {
        ...
        *getThirdParty({ payload }, { call, put }) {
            let res = yield call(getThirdParty, payload);
            if (res.ok) {
