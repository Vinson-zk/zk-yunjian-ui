...
import { editThirdPartyAuthAccountUserGps, delThirdPartyAuthAccountUserGps, getThirdPartyAuthAccountUserGps, findThirdPartyAuthAccountUserGpss } from './service';

import { zkTools } from 'zkFramework';
const { zkToolsUtils, zkToolsMsg } = zkTools;

const model = {
    namespace: 'mThirdPartyAuthAccountUserGps',
    state: {
    ...
    effects: { // action
        // 编辑 
        *editThirdPartyAuthAccountUserGps({ payload, callback }, { call }) {
            let res = yield call(editThirdPartyAuthAccountUserGps, payload);
            let f = errors=>{
        ...
        // 删除
        *delThirdPartyAuthAccountUserGps({ payload, callback }, { call }) {
            let res = yield call(delThirdPartyAuthAccountUserGps, payload);
            if(res.code == "zk.0"){
                zkToolsMsg.alertMsg(null, null, {type:"success", msg:res.msg});
            }
            if (callback instanceof Function) {
        ...
        // 查询 详情
        *getThirdPartyAuthAccountUserGps({ payload }, { call, put }) {
            let res = yield call(getThirdPartyAuthAccountUserGps, payload);
            if (res.code == 'zk.0') {
         ...
         */
        *findThirdPartyAuthAccountUserGpss({ filter, page, sorter, callback }, { call, put, select }) {
            ...
            let res = yield call(findThirdPartyAuthAccountUserGpss, params);
            let restState = {}
            if (res.code == 'zk.0') {
                restState = {
                    "filter": params,
