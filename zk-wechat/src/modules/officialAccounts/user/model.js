 ...
 * @Last Modified time: 2024-06-24 17:44:37
 */

import { editOfficialAccountsUser, delOfficialAccountsUser, getOfficialAccountsUser, findOfficialAccountsUsers } from './service';

import { zkTools } from 'zkFramework';
const { zkToolsUtils, zkToolsMsg } = zkTools;

const model = {
    namespace: 'mOfficialAccountsUser',
    state: {
    ...
    effects: { // action
        // 编辑 
        *editOfficialAccountsUser({ payload, callback }, { call }) {
            let res = yield call(editOfficialAccountsUser, payload);
            let f = errors=>{
        ...
        // 删除
        *delOfficialAccountsUser({ payload, callback }, { call }) {
            let res = yield call(delOfficialAccountsUser, payload);
            if(res.ok){
                zkToolsMsg.alertMsg(null, null, {type:"success", msg:res.msg});
            }
            if (zkJsUtils.assertObjType(callback, Function)) {
        ...
        // 查询 详情
        *getOfficialAccountsUser({ payload }, { call, put }) {
            let res = yield call(getOfficialAccountsUser, payload);
            if (res.ok) {
         ...
         */
        *findOfficialAccountsUsers({ filter, pagination, sorter, callback }, { call, put, select }) {
            let params = zkToolsUtils.convertSortParam(filter, sorter); 
            if(pagination){
                params = { ...params, ...zkToolsUtils.convertPageParam(pagination) };
            }
            let res = yield call(findOfficialAccountsUsers, params);
            let restState = {}
            if (res.ok) {
                restState = {
                    "filter": params,
