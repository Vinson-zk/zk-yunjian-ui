 ...
 * @Last Modified time: 2024-06-24 17:48:54
 */

import { editFuncKeyConfig, delFuncKeyConfig, getFuncKeyConfig, findFuncKeyConfigs } from './service';

import { zkTools } from 'zkFramework';
const { zkToolsUtils, zkToolsMsg } = zkTools;

const model = {
    namespace: 'mFuncKeyConfig',
    state: {
    ...
    effects: { // action
        // 编辑 
        *editFuncKeyConfig({ payload, callback }, { call }) {
            let res = yield call(editFuncKeyConfig, payload);
            let f = errors=>{
        ...
        // 删除
        *delFuncKeyConfig({ payload, callback }, { call }) {
            let res = yield call(delFuncKeyConfig, payload);
            if(res.ok){
                zkToolsMsg.alertMsg(null, null, {type:"success", msg:res.msg});
            }
            if (zkJsUtils.assertObjType(callback, Function)) {
        ...
        // 查询 详情
        *getFuncKeyConfig({ payload }, { call, put }) {
            let res = yield call(getFuncKeyConfig, payload);
            if (res.ok) {
         ...
         */
        *findFuncKeyConfigs({ filter, pagination, sorter, callback }, { call, put, select }) {
            let params = zkToolsUtils.convertSortParam(filter, sorter); 
            if(pagination){
                params = { ...params, ...zkToolsUtils.convertPageParam(pagination) };
            }
            let res = yield call(findFuncKeyConfigs, params);
            let restState = {}
            if (res.ok) {
                restState = {
                    "filter": params,
