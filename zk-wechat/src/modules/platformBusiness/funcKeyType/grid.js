...
const f_getTableColumns = (onEedit, onDetail, onDelete, intl, lang) => {
	...
	return [
		{
			title: zkToolsMsg.msgFormatByIntl(intl, 'zk.wechat.platformBusiness.funcKeyType.funcTypeName'),
			textAlign: 'center', dataIndex: 'funcTypeName', key: 'funcTypeName', width: 100, 
			render: (text, record, index) => {
				return zkToolsMsg.getInternationInfo(record.funcTypeName?record.funcTypeName:{}, lang);
			}
		},
		{
			title: zkToolsMsg.msgFormatByIntl(intl, 'zk.wechat.platformBusiness.funcKeyType.funcTypeCode'),
			textAlign: 'center', dataIndex: 'funcTypeCode', key: 'funcTypeCode', width: 100, 
		},
		{
			title: zkToolsMsg.msgFormatByIntl(intl, 'zk.wechat.platformBusiness.funcKeyType.status'),
			textAlign: 'center', dataIndex: 'status', key: 'status', width: 100, 
			render: (text, record, index) => {
				return zkToolsMsg.msgFormatByIntl(intl, 'zk.wechat.platformBusiness.funcKeyType.status.' + text);
			}
		},
		{
			title: zkToolsMsg.msgFormatByIntl(intl, 'zk.wechat.platformBusiness.funcKeyType.funcTypeDesc'),
			textAlign: 'center', dataIndex: 'funcTypeDesc', key: 'funcTypeDesc', width: 100, 
			render: (text, record, index) => {
				return zkToolsMsg.getInternationInfo(record.funcTypeDesc?record.funcTypeDesc:{}, lang);
			}
		},
		{
...
class CInitFuncKeyTypeGrid extends React.Component {
	...
	f_delete = (keys, isConfirm) => {
		...
		let f_executeDelete = (pkIds) => {
            _this.props.dispatch({
                type: "mFuncKeyType/delFuncKeyType", payload: { pkId: pkIds },
                callback: () => {
                    _this.props.dispatch({ type: 'mFuncKeyType/findFuncKeyTypes', filter: _this.props.mFuncKeyType.filter, pagination: mFuncKeyType.pagination, callback: e => { } })
                }
            });
		};
	...
	f_changeSelKeys = (selRowKeys, selRows) => {
		this.props.dispatch({ type: "mFuncKeyType/setState", payload: { gridSelKeys: selRowKeys } });
	}
	...
	f_changeGrid = (pagination, filters, sorter) => {
        ...
        this.props.dispatch({ 
            ...
            filter: this.props.mFuncKeyType.filter,
	...
	render() {
		...
		let { intl, mApp, mFuncKeyType, loading } = this.props;
        let lang = mApp.lang?mApp.lang:zkToolsMsg.getLocale();

		let tableColumns = f_getTableColumns(this.f_edit, this.f_detail, this.f_delete, intl, lang);

		let gridLoading = loading.effects['mFuncKeyType/findFuncKeyTypes'];
		...
		return (
			<ZKScrollTable loading = { gridLoading }
				...
				rowSelection = {{
					...
					selectedRowKeys: mFuncKeyType.gridSelKeys||[], 
					columnWidth: '32px'
				}}
				rowKey = "pkId"
			...
			>
				<ZKOptRow>
					<ZKOptRow.OptGroup>
						...
						<ZKOptRow.OptGroup.OptItem onClick={(e) => {
							// 删除
							this.f_delete(mFuncKeyType.gridSelKeys, true);
...
export default CInitFuncKeyTypeGrid;