 ...
 * @Last Modified time: 2024-06-24 17:49:00
 */

import { editFuncKeyType, delFuncKeyType, getFuncKeyType, findFuncKeyTypes } from './service';

import { zkTools } from 'zkFramework';
const { zkToolsUtils, zkToolsMsg } = zkTools;

const model = {
    namespace: 'mFuncKeyType',
    state: {
    ...
    effects: { // action
        // 编辑 
        *editFuncKeyType({ payload, callback }, { call }) {
            let res = yield call(editFuncKeyType, payload);
            let f = errors=>{
        ...
        // 删除
        *delFuncKeyType({ payload, callback }, { call }) {
            let res = yield call(delFuncKeyType, payload);
            if(res.ok){
                zkToolsMsg.alertMsg(null, null, {type:"success", msg:res.msg});
            }
            if (zkJsUtils.assertObjType(callback, Function)) {
        ...
        // 查询 详情
        *getFuncKeyType({ payload }, { call, put }) {
            let res = yield call(getFuncKeyType, payload);
            if (res.ok) {
         ...
         */
        *findFuncKeyTypes({ filter, pagination, sorter, callback }, { call, put, select }) {
            let params = zkToolsUtils.convertSortParam(filter, sorter); 
            if(pagination){
                params = { ...params, ...zkToolsUtils.convertPageParam(pagination) };
            }
            let res = yield call(findFuncKeyTypes, params);
            let restState = {}
            if (res.ok) {
                restState = {
                    "filter": params,
