 ...
 * @Last Modified time: 2023-09-23 23:52:35
 */
...
const FInitJsEventDemo = ({ intl }) => {
	...
	return (
		<ZKContentFormat className={styles.sample_detail_panel} >
			<ZKContentFormat title = {`${zkToolsMsg.msgFormatByIntl(intl, 'sample.general.function')} zkJsEvent ${zkToolsMsg.msgFormatByIntl(intl, 'global.app.info.demo')}`}>
			...
			<ZKContentFormat title = {`事件订阅发布类 ${zkToolsMsg.msgFormatByIntl(intl, 'global.app.info.declare')}`} >
				<div id="eventEmitte">
					<SyntaxHighlighter language='jsx' style={docco} className={`${styles.zk_SyntaxHighlighter}`}>
						{[
							"自定义事件发布、订阅、派发类",
			...
			<ZKContentFormat title = {`js 事件处理通用方法 ${zkToolsMsg.msgFormatByIntl(intl, 'global.app.info.declare')}`} >
				<div id="eventBinding">
					<SyntaxHighlighter language='jsx' style={docco} className={`${styles.zk_SyntaxHighlighter}`}>
						{[
							"/** js 注册事件监听 通用方法 ",
			...
			<ZKContentFormat title = {`js 事件 笔记 ${zkToolsMsg.msgFormatByIntl(intl, 'global.app.info.declare')}`} >
				<SyntaxHighlighter language='jsx' style={docco} className={`${styles.zk_SyntaxHighlighter}`}>
					{[
						"事件捕获 <div><p></p></div>",
