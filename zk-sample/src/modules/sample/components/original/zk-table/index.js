 ...
 * @Last Modified time: 2024-07-04 16:01:15
 */
...
const { ZKContentFormat, ZKOptRow } = ZKCustomComponents;
...
function FInitZKTableDemo({ intl }) {
	...
	let tableProps = {
		...
		rowSelection: { 
			...
			onCell: (record, rowIndex)	 => { // 设置单元格属性，用法与 Column 的 onCell 相同 -	5.5.0
				// console.log("[^_^:20230907-0102-001] rowSelection.onCell: ", record, rowIndex);
			},
			onChange: (selectedRowKeys, selectedRows, info)=>{ // 选中项发生变化时的回调	function	-	info.type: 4.21.0
				console.log("[^_^:20230907-0102-002] rowSelection.onChange: ", selectedRowKeys, selectedRows, info);
			},
		...
		onChange: (pagination, filters, sorter)=>{
			console.log("[^_^:20230907-0101-001] pagination: ", pagination);
			console.log("[^_^:20230907-0101-001] filters: ", filters);
			console.log("[^_^:20230907-0101-001] sorter: ", sorter);
			/*
			{
				column:{
	...
	return (
		<ZKContentFormat className={styles.sample_detail_panel} >
			<ZKContentFormat title = {`${zkToolsMsg.msgFormatByIntl(intl, 'sample.components.original.talbe')} ${zkToolsMsg.msgFormatByIntl(intl, 'global.app.info.demo')}`}>
				<ZKTable {...tableProps} columns = {columns1}></ZKTable>
				<br />
				<ZKTable {...tableProps} columns = {columns2} rowNum={undefined} ></ZKTable>
				<br />
