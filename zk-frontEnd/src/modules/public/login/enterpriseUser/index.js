...
const FInitEnterpriseLoginForm = ({locales, intl, match, dispatch, history, mApp, onChangeUserType, className})=>{
	...
	return (
	    <div className = {`${loginStyles.login_panel} ${className}`} >
    		<ZKTabs type="card" tabBarGutter = {2} className = {frontEndStyles.zk_public_tabs} items = {items}
    			tabBarExtraContent = {
            ...
            <ZKRow gutter={24} >
                <ZKCol span = {24} className = {loginStyles.login_no_account_register} >
                    &nbsp;{zkToolsMsg.msgFormatByIntl(intl, 'zk.front.end.login.lable.register.guide')}<Link to={`/_register_company/0`}>{zkToolsMsg.msgFormatByIntl(intl, 'zk.front.end.login.lable.register')}</Link>
                </ZKCol>
            </ZKRow>
			<ZKRow gutter={24} >
				<ZKDivider className = {loginStyles.login_divider} />
				<ZKCol span = {12}>
                    <Link to={`/_forgot_password/company`}>[{ zkToolsMsg.msgFormatByIntl(intl, 'zk.front.end.login.lable.forgotPassword') }]</Link>
                </ZKCol>
...
export default FInitEnterpriseLoginForm;
