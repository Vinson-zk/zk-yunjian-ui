 ...
 * @Last Modified time: 2023-10-08 17:59:25
 */
...
class CInitSider extends React.Component {
	...
	f_toggle = (e)=>{
		...
		if(zkJsUtils.assertObjType(this.props.onCollapsedChange, Function)){
	...
	render() {
		...
		return (
			<Sider { ...resProps } ref = {this.thisRef} className={`${styles.zk_slider} ${className ? className : ""}`}
				trigger={<div>sss</div>}
			...
			>
				<div className = { styles.zk_slider_head }>
				{ this.state.collapsed ?
					<MenuUnfoldOutlined className={`${styles.zk_slider_trigger} ${styles.zk_slider_trigger_open}`} onClick={ this.f_toggle } />
					: <MenuFoldOutlined className={`${styles.zk_slider_trigger} ${styles.zk_slider_trigger_open}`} onClick={ this.f_toggle } />
				}
				</div>
				{children}
			</Sider>
