...
* @Last Modified time: 2023-10-08 18:00:55
*/
...
class CInitTouchControl extends React.Component {
	...
	constructor(props) {
		...
		if(props.schema === "nav"){
			_className = `${_className} ${styles.zk_touch_nav}`;
			switch(props.navDirection){
				case 'left': 
					_className = `${_className} ${styles.zk_touch_nav_left}`;
					break;
				case 'top': 
					_className = `${_className} ${styles.zk_touch_nav_top}`;
					break;
				case 'right': 
					_className = `${_className} ${styles.zk_touch_nav_right}`;
					break;
				case 'bottom': 
					_className = `${_className} ${styles.zk_touch_nav_bottom}`;
					break;
			}
		}else{
			_className = `${_className} ${styles.zk_touch_default}`;
	...
	f_touch = (e)=>{
		if(zkJsUtils.assertObjType(this.props.onTouch, Function)){
	...
	// 鼠标进入
	f_mouseover = e=>{
		if(zkJsUtils.assertObjType(this.props.onMouseover, Function)){
    		this.props.onMouseover.call(this, e, this.props.data);
    	}
	};
	// 鼠标离开
	f_mouseout = e=>{
		if(zkJsUtils.assertObjType(this.props.onMouseout, Function)){
	...
	render() {
		...
		if(!this.state.mouseMove){
			_className = `${_className} ${styles.zk_touch_nav_move_transition}`;
		}
