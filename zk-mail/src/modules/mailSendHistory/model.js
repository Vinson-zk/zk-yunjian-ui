 ...
 * @Last Modified time: 2024-06-24 17:39:39
 */

import { editMailSendHistory, delMailSendHistory, getMailSendHistory, findMailSendHistorys } from './service';

import { zkTools } from 'zkFramework';
const { zkToolsUtils, zkToolsMsg } = zkTools;

const model = {
    namespace: 'mMailSendHistory',
    state: {
    ...
    effects: { // action
        // 编辑 
        *editMailSendHistory({ payload, callback }, { call }) {
            let res = yield call(editMailSendHistory, payload);
            let f = errors=>{
        ...
        // 删除
        *delMailSendHistory({ payload, callback }, { call }) {
            let res = yield call(delMailSendHistory, payload);
            if(res.ok){
                zkToolsMsg.alertMsg(null, null, {type:"success", msg:res.msg});
            }
            if (zkJsUtils.assertObjType(callback, Function)) {
        ...
        // 查询 详情
        *getMailSendHistory({ payload }, { call, put }) {
            let res = yield call(getMailSendHistory, payload);
            if (res.ok) {
         ...
         */
        *findMailSendHistorys({ filter, pagination, sorter, callback }, { call, put, select }) {
            let params = zkToolsUtils.convertSortParam(filter, sorter); 
            if(pagination){
                params = { ...params, ...zkToolsUtils.convertPageParam(pagination) };
            }
            let res = yield call(findMailSendHistorys, params);
            let restState = {}
            if (res.ok) {
                restState = {
                    "filter": params,
