...
const f_getTableColumns = (onEedit, onDetail, onDelete, intl, lang) => {
	...
	return [
		{
			title: zkToolsMsg.msgFormatByIntl(intl, 'zk.mail.MailType.typeCode'),
			textAlign: 'center', dataIndex: 'typeCode', key: 'typeCode', width: 100, 
		},
		{
			title: zkToolsMsg.msgFormatByIntl(intl, 'zk.mail.MailType.typeName'),
		...
		{
			title: zkToolsMsg.msgFormatByIntl(intl, 'zk.mail.MailType.status'),
			textAlign: 'center', dataIndex: 'status', key: 'status', width: 100, 
			render: (text, record, index) => {
				return zkToolsMsg.msgFormatByIntl(intl, 'zk.mail.MailType.status.' + text);
			}
		},
		{
...
class CInitMailTypeGrid extends React.Component {
	...
	f_delete = (keys, isConfirm) => {
		...
		let f_executeDelete = (pkIds) => {
            _this.props.dispatch({
                type: "mMailType/delMailType", payload: { pkId: pkIds },
                callback: () => {
                    _this.props.dispatch({ type: 'mMailType/findMailTypes', filter: _this.props.mMailType.filter, pagination: mMailType.pagination, callback: e => { } })
                }
            });
		};
	...
	f_changeSelKeys = (selRowKeys, selRows) => {
		this.props.dispatch({ type: "mMailType/setState", payload: { gridSelKeys: selRowKeys } });
	}
	...
	f_changeGrid = (pagination, filters, sorter) => {
        ...
        this.props.dispatch({ 
            ...
            filter: this.props.mMailType.filter,
	...
	render() {
		...
		let { intl, mApp, mMailType, loading } = this.props;
        let lang = mApp.lang?mApp.lang:zkToolsMsg.getLocale();

		let tableColumns = f_getTableColumns(this.f_edit, this.f_detail, this.f_delete, intl, lang);

		let gridLoading = loading.effects['mMailType/findMailTypes'];
		...
		return (
			<ZKScrollTable loading = { gridLoading }
				...
				rowSelection = {{
					...
					selectedRowKeys: mMailType.gridSelKeys||[], 
					columnWidth: '32px'
				}}
				rowKey = "pkId"
				rowNum = {{'textAlign': 'center', 'fixed': 'left', width: 40}}
				columns = {tableColumns}
				scroll = {{ x:900, y: this.state.sh }}
				pagination = {mMailType.page||{}}
				// pagination = {{position: ['topRight'], ...page}}
                dataSource = {mMailType.gridData||[]}
			...
			>
				<ZKOptRow>
					<ZKOptRow.OptGroup>
						...
						<ZKOptRow.OptGroup.OptItem onClick={(e) => {
							// 删除
							this.f_delete(mMailType.gridSelKeys, true);
...
export default CInitMailTypeGrid;