...
class CInitMailTypeDetail extends Component {
  ...
  render() {
    ...
    return (optEntity != undefined && mMailType.pathname == location.pathname) && (
  ...
  componentDidMount() {
    let { location, dispatch, match, mMailType } = this.props;
    let { params } = match;
...
export default injectIntl(connect(({ mApp, mMailType, loading }) => ({ mApp, mMailType, loading }))(CInitMailTypeDetail));