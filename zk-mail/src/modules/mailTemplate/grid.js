...
const f_getTableColumns = (onEedit, onDetail, onDelete, intl, lang) => {
	...
	return [
		{
			title: zkToolsMsg.msgFormatByIntl(intl, 'zk.mail.MailTemplate.typeCode'),
			textAlign: 'center', dataIndex: 'typeCode', key: 'typeCode', width: 100, 
		},
		{
			title: zkToolsMsg.msgFormatByIntl(intl, 'zk.mail.MailTemplate.companyCode'),
			textAlign: 'center', dataIndex: 'companyCode', key: 'companyCode', width: 100, 
		},
		{
			title: zkToolsMsg.msgFormatByIntl(intl, 'zk.mail.MailTemplate.locale'),
		...
		{
			title: zkToolsMsg.msgFormatByIntl(intl, 'zk.mail.MailTemplate.status'),
			textAlign: 'center', dataIndex: 'status', key: 'status', width: 100, 
			render: (text, record, index) => {
				return zkToolsMsg.msgFormatByIntl(intl, 'zk.mail.MailTemplate.status.' + text);
			}
		},
		{
			title: zkToolsMsg.msgFormatByIntl(intl, 'zk.mail.MailTemplate.sendAddress'),
			textAlign: 'center', dataIndex: 'sendAddress', key: 'sendAddress', width: 100, 
		},
		{
			title: zkToolsMsg.msgFormatByIntl(intl, 'zk.mail.MailTemplate.sendName'),
			textAlign: 'center', dataIndex: 'sendName', key: 'sendName', width: 100, 
		},
		{
			title: zkToolsMsg.msgFormatByIntl(intl, 'zk.mail.MailTemplate.subject'),
...
class CInitMailTemplateGrid extends React.Component {
	...
	f_delete = (keys, isConfirm) => {
		...
		let f_executeDelete = (pkIds) => {
            _this.props.dispatch({
                type: "mMailTemplate/delMailTemplate", payload: { pkId: pkIds },
                callback: () => {
                    _this.props.dispatch({ type: 'mMailTemplate/findMailTemplates', filter: _this.props.mMailTemplate.filter, pagination: mMailTemplate.pagination, callback: e => { } })
                }
            });
		};
	...
	f_changeSelKeys = (selRowKeys, selRows) => {
		this.props.dispatch({ type: "mMailTemplate/setState", payload: { gridSelKeys: selRowKeys } });
	}
	...
	f_changeGrid = (pagination, filters, sorter) => {
        ...
        this.props.dispatch({ 
            ...
            filter: this.props.mMailTemplate.filter,
	...
	render() {
		...
		let { intl, mApp, mMailTemplate, loading } = this.props;
        let lang = mApp.lang?mApp.lang:zkToolsMsg.getLocale();

		let tableColumns = f_getTableColumns(this.f_edit, this.f_detail, this.f_delete, intl, lang);

		let gridLoading = loading.effects['mMailTemplate/findMailTemplates'];
		...
		return (
			<ZKScrollTable loading = { gridLoading }
				...
				rowSelection = {{
					...
					selectedRowKeys: mMailTemplate.gridSelKeys||[], 
					columnWidth: '32px'
				}}
				rowKey = "pkId"
				...
				pagination = {mMailTemplate.page||{}}
				// pagination = {{position: ['topRight'], ...page}}
                dataSource = {mMailTemplate.gridData||[]}
			...
			>
				<ZKOptRow>
					<ZKOptRow.OptGroup>
						...
						<ZKOptRow.OptGroup.OptItem onClick={(e) => {
							// 删除
							this.f_delete(mMailTemplate.gridSelKeys, true);
...
export default CInitMailTemplateGrid;