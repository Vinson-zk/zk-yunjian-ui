...
class CInitTableInfo extends Component {
  ...
  onUpdateTableList = ()=>{
    this.props.dispatch({ type: "mTableInfo/updateTableList", moduleId: this.props.mFuncModule.optEntity.pkId, callback:()=>{
      this.props.dispatch({ type: 'mTableInfo/getTables', moduleId: this.props.mFuncModule.optEntity.pkId, forceUpdate:true });
    } });
  }
  // 删除表信息，级联删除表信息字段
  onDelete = (keys, isConfirm)=>{
    ...
    let spanLoading = loading.effects['mTableInfo/getTables'] || loading.effects["mTableInfo/editTableInfo"] || loading.effects["mTableInfo/updateTableInfo"] 
      || loading.effects["mGen/genCode"] || loading.effects["mTableInfo/updateTableList"] || loading.effects["mTableInfo/delTableInfo"];
        ...
        >
          <div className = {styles.title_row} >
            ...
            <div className = {styles.title_opt}>
              ...
              <ZKButton onClick={this.onUpdateTableList} >{zkToolsMsg.msgFormatByIntl(intl, 'zk.codeGen.opt._key_update_tableInfo_list')}</ZKButton>
              <ZKButton onClick={ (e) => { this.onDelete(this.state.selKeys, true)}} >{zkToolsMsg.msgFormatByIntl(intl, 'zk.codeGen.opt._key_del_tableInfo')}</ZKButton>
            </div>
          </div>
        </ZKScrollTable>
      </ZKForm>
