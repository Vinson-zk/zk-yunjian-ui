...
* @QQ: 1035862795
* @Wechat: 1035862795
* @Date: 2025-01-08 17:11:13
* @Last Modified by: vinson
* @Last Modified time: 2025-01-08 17:15:41
*/


import { sEditIotProdInstance, sDelIotProdInstance, sGetIotProdInstance, sFindIotProdInstances } from './service';

import { zkTools } from 'zkFramework';
const { zkToolsUtils, zkToolsMsg } = zkTools;

const model = {
    namespace: 'mIotProdInstance',
    state: {
    ...
    effects: { // action
        // 编辑 
        *editIotProdInstance({ payload, callback }, { call }) {
            let res = yield call(sEditIotProdInstance, payload);
            let f = errors=>{
        ...
        // 删除
        *delIotProdInstance({ payload, callback }, { call }) {
            let res = yield call(sDelIotProdInstance, payload);
            if(res.code == "zk.0"){
                zkToolsMsg.alertMsg(null, null, {type:"success", msg:res.msg});
            }
            if (callback instanceof Function) {
        ...
        // 查询 详情
        *getIotProdInstance({ payload }, { call, put }) {
            let res = yield call(sGetIotProdInstance, payload);
            if (res.code == 'zk.0') {
         ...
         */
        *findIotProdInstances({ filter, pagination, sorter, callback }, { call, put, select }) {
            let params = zkToolsUtils.convertSortParam(filter, sorter); 
            if(pagination){
                params = { ...params, ...zkToolsUtils.convertPageParam(pagination) };
            }
            let res = yield call(sFindIotProdInstances, params);
            let restState = {}
            if (res.code == 'zk.0') {
                restState = {
                    "filter": params,
