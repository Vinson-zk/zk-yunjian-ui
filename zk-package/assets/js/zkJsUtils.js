  ...
  */
  ...
  Array.prototype.indexOf = function (val, f_compare) {
      for (let i = 0; i < this.length; i++) {
          if(f_assertObjType(f_compare, Function)){
  ...
  /** 判断一个对象是否为空
   * @obj: 待判断的对象;
   * @isExtend: 扩展判断，true-时，'' 返回 true;
   * @return: 返回：null-true、undefined-true
   */
  const f_isEmpty = (obj, isExtend=true) => {
  ...
  const f_makeTree = (objList, rootId, keyId="pkId", keyParentId="parentId") => {
      let outTreeObjs = [];
      let otherObjs = [];
      if (f_assertObjType(objList, Array)) {
          if (rootId === undefined) {
              // 找出所有根结点；
  ...
  const f_findTreeParent = (treeDatas=[], childId) => {
    ...
    for (let item of treeDatas) {
      if (item.pkId == childId) {
        ...
        if (f_assertObjType(item.children, Array)) {
  ...
  const f_strReplace = (msg, opt) => {
      ...
      if(f_assertObjType(msg, String) && f_assertObjType(opt, Object)){
          for (let arg in opt) {
  ...
  const f_sort = (items, flag = 1, compareFunc, optChildren = 1) => {
      ...
      // 先升序排出序顺
      let sortItems = [] // 结果数组
      if (f_assertObjType(items, Array) && items.length > 0) {
          let item = null
  ...
  const f_clone = obj=>{
      ...
      if(f_assertObjType(obj, Object) && f_assertObjType(obj, Array)){
          return [].concat(obj);
      }
      if (obj != null && f_assertObjType(obj, Object)) {
   ...
   * @keys: 修改的 css 变量名， 不带 '--'
   */
  const f_changeStyleCssVal = (valMap, keys=[]) => {
    for (let key of keys) {
      document.getElementsByTagName("body")[0].style.setProperty(`--${key}`, valMap[key]);
    }
  };
   ...
   */
  const f_assertObjType = (obj, type) => {
    if(f_isEmpty(obj, false)){
      return false;
    }
  ...
  const _zkJsUtils = {
    ...
    changeStyleCssVal: f_changeStyleCssVal,  // 修改页面中的样式变量值
    assertObjType: f_assertObjType, 
  } 
