 ...
 * @Last Modified time: 2024-06-27 22:36:08
 */
...
if ("start" == command) {
    ...
    // 设置默认配置或统一设置一些配置
    devServerOptions = Object.assign(
        {
            host: 'localhost', 
            port: 8000, 
    ...
    // 加监听，输出启动后的访问地址
    // let onListening = devServerOptions.onListening;
    // devServerOptions.onListening = (devServer)=>{
    //     if(onListening instanceof Function){
    //         onListening.call(this, devServer);
    //     }
    //     console.log(`[^_^:20190322-1804-003] Starting project [${projectName}] server on http://${devServerOptions.host}:${devServerOptions.port}`);
    // }

    /*** 启动项目 ***/
    // console.log("[^_^:20200810-1410-001] ------------------------------");
    // console.log(`启动了项目:${projectName}; devServerOptions:\n`, devServerOptions);
    const compiler = webpack(webpackConfig);
    const server = new webpackDevServer(devServerOptions, compiler);

    // console.log("[^_^:20240627-2236-001] server: ", server);
    ...
    const runServer = async () => {
        console.log(`[^_^:20240627-2231-001] Starting project [${projectName}] server...`);
        console.log(`[^_^:20240627-2231-001] http://${devServerOptions.host}:${devServerOptions.port}`);
        await server.start();
    };
    runServer();
